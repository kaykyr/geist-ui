...
describe('Slider', () => {
	...
	it('should trigger events when click', async () => {
		...
		const wrapper = mount(
			<Slider initialValue={20} onChange={changeHandler} />,
		)
	...
	it('should trigger events when drag', async () => {
		...
		const changeHandler = jest
			...
			.mockImplementation((val) => (value = val))
		const wrapper = mount(
			<Slider initialValue={0} onChange={changeHandler} />,
		)
	...
	it('should ignore events when disabled', async () => {
		...
		const changeHandler = jest
			...
			.mockImplementation((val) => (value = val))
		const wrapper = mount(
			<Slider initialValue={0} disabled onChange={changeHandler} />,
		)
		const dot = wrapper.find('.dot').getDOMNode() as HTMLDivElement
	...
	it('should return the specified when the limit is exceeded', () => {
		...
		const changeHandler = jest
			...
			.mockImplementation((val) => (value = val))
		const wrapper = mount(
			<Slider min={10} max={20} onChange={changeHandler} />,
		)
		const dot = wrapper.find('.dot').getDOMNode() as HTMLDivElement
