...
const BaseComponent: React.FC<React.PropsWithChildren<unknown>> = ({
	children,
}) => <div>{children}</div>
...
describe('UseScale', () => {
	...
	it('should work correctly with SCALES', () => {
		...
		const { result, rerender } = renderHook<ScaleProps, ScaleConfig>(
			() => useScale(),
			{
				wrapper,
				initialProps: {},
	...
	it('should be update all native scale props', () => {
		...
		const { result, rerender } = renderHook<any, ScaleConfig>(
			() => useScale(),
			{
				wrapper,
				initialProps: {},
