		...
		files
			...
			.map(async (file) => {
				...
				if (isDirectory) {
					...
					const childrenMetadata = await getMetadata(
						children,
						filePath,
					)
					const sorted = childrenMetadata.sort(
						(a, b) => a.index - b.index,
					)
					...
					if (childrenHasGroup) {
						const groups = [
							...new Set(
								sorted.map((item) => item.group || 'others'),
							),
						]
		...
		const sortdMetaData = await Promise.all(
			locales.map(async (name) => {
				...
				const sorted = data.sort(
					(a, b) => weights[a.name] - weights[b.name],
				)
